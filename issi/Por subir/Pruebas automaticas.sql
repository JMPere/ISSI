...
/*PAQUETE PRUEBAS CARRITO*/
CREATE OR REPLACE PACKAGE PRUEBAS_CARRITO AS
    PROCEDURE INICIALIZAR;
    PROCEDURE INSERTAR( NOMBRE_PRUEBA VARCHAR2, 
                        W_NOMBREMUEBLE IN CARRITO.NOMBREMUEBLE%TYPE,
                        W_NICKNAME_CLIENTE IN CARRITO.NICKNAME_CLIENTE%TYPE,
                        W_CANTIDAD IN CARRITO.CANTIDAD%TYPE,
                        W_PRECIO IN CARRITO.PRECIO%TYPE,
                        SALIDA_ESPERADA BOOLEAN);
...
create or replace PACKAGE BODY PRUEBAS_CARRITO AS
    ...
    AS
        SALIDA BOOLEAN := TRUE;
        TIPO_CARRITO CARRITO%ROWTYPE;

    BEGIN
        INSERT INTO CARRITO VALUES (W_NOMBREMUEBLE, W_NICKNAME_CLIENTE, W_PRECIO, W_CANTIDAD);

        SELECT * INTO TIPO_CARRITO FROM CARRITO WHERE NOMBREMUEBLE = W_NOMBREMUEBLE;
        IF (TIPO_CARRITO.NOMBREMUEBLE <> W_NOMBREMUEBLE OR TIPO_CARRITO.NICKNAME_CLIENTE <> W_NICKNAME_CLIENTE) THEN
            SALIDA := FALSE;
        END IF;
        COMMIT WORK;
    ...
    BEGIN
        ...
        UPDATE CARRITO
            SET NICKNAME_CLIENTE = W_NICKNAME_CLIENTE,
                CANTIDAD = W_CANTIDAD,
                PRECIO = W_PRECIO
            WHERE NOMBREMUEBLE = W_NOMBREMUEBLE;

        SELECT * INTO TIPO_CARRITO FROM CARRITO WHERE NOMBREMUEBLE = W_NOMBREMUEBLE;

        IF (TIPO_CARRITO.NOMBREMUEBLE <> W_NOMBREMUEBLE
            OR TIPO_CARRITO.NICKNAME_CLIENTE <> W_NICKNAME_CLIENTE
            OR TIPO_CARRITO.PRECIO <> W_PRECIO
            OR TIPO_CARRITO.CANTIDAD <> W_CANTIDAD ) THEN
    ...
    BEGIN
        DELETE FROM CARRITO WHERE NOMBREMUEBLE = W_NOMBREMUEBLE;

        SELECT COUNT(*) INTO N_CARRITO FROM CARRITO WHERE NOMBREMUEBLE = W_NOMBREMUEBLE;
        IF (N_CARRITO <> 0) THEN
            SALIDA := FALSE;
        END IF;
        COMMIT WORK;
...
/*PAQUETE PRUEBAS VENTA*/
CREATE OR REPLACE PACKAGE PRUEBAS_VENTA AS
    PROCEDURE INICIALIZAR;
    PROCEDURE INSERTAR( NOMBRE_PRUEBA VARCHAR2, 
                        W_NICKNAME_CLIENTE IN VENTA.NICKNAME_CLIENTE%TYPE,
                        W_NOMBREMUEBLE IN VENTA.NOMBREMUEBLE%TYPE,
                        W_CANTIDADMUEBLE IN VENTA.CANTIDADMUEBLE%TYPE,
                        W_FECHACOMPRA IN VENTA.FECHACOMPRA%TYPE,
                        W_PRECIOTOTAL IN VENTA.PRECIOTOTAL%TYPE,
                        W_NUMPLAZOS IN VENTA.NUMPLAZOS%TYPE,
                        SALIDA_ESPERADA BOOLEAN);
...
CREATE OR REPLACE PACKAGE BODY PRUEBAS_VENTA AS
    ...
    AS
        SALIDA BOOLEAN := TRUE;
        TIPO_VENTA VENTA%ROWTYPE;
        W_OID_VENTA VENTA.OID_VENTA%TYPE;
        
    BEGIN
        INSERT INTO VENTA (NICKNAME_CLIENTE, NOMBREMUEBLE, CANTIDADMUEBLE, FECHACOMPRA, PRECIOTOTAL, NUMPLAZOS) 
            VALUES (W_NICKNAME_CLIENTE, W_NOMBREMUEBLE, W_CANTIDADMUEBLE, W_FECHACOMPRA, W_PRECIOTOTAL, W_NUMPLAZOS);

        W_OID_VENTA := INCRE_OID_VENTA.CURRVAL;
        SELECT * INTO TIPO_VENTA FROM VENTA WHERE OID_VENTA = W_OID_VENTA;
        IF (TIPO_VENTA.OID_VENTA <> W_OID_VENTA) THEN
            SALIDA := FALSE;
        END IF;
        COMMIT WORK;
    ...
    BEGIN
        ...
        UPDATE VENTA
            SET NICKNAME_CLIENTE = W_NICKNAME_CLIENTE,
                NOMBREMUEBLE = W_NOMBREMUEBLE,
                CANTIDADMUEBLE = W_CANTIDADMUEBLE,
                FECHACOMPRA = W_FECHACOMPRA,
                PRECIOTOTAL = W_PRECIOTOTAL,
                NUMPLAZOS = W_NUMPLAZOS              
            WHERE OID_VENTA = W_OID_VENTA;
            
        SELECT * INTO TIPO_VENTA FROM VENTA WHERE OID_VENTA = W_OID_VENTA;
    
        IF (TIPO_VENTA.NICKNAME_CLIENTE <> W_NICKNAME_CLIENTE
            OR TIPO_VENTA.NOMBREMUEBLE <> W_NOMBREMUEBLE
            OR TIPO_VENTA.CANTIDADMUEBLE <> W_CANTIDADMUEBLE
            OR TIPO_VENTA.FECHACOMPRA <> W_FECHACOMPRA
            OR TIPO_VENTA.PRECIOTOTAL <> W_PRECIOTOTAL
            OR TIPO_VENTA.NUMPLAZOS <> W_NUMPLAZOS ) THEN
    ...
    BEGIN
        ...
        SELECT COUNT(*) INTO N_VENTA FROM VENTA WHERE OID_VENTA = W_OID_VENTA;
        IF (N_VENTA <> 0) THEN
            SALIDA := FALSE;
        END IF;
        COMMIT WORK;
...
BEGIN
    ...
    PRUEBAS_CLIENTE.INSERTAR('INSERTA CLIENTE', '12345678L', 'MONOLO', 'WERTYUASC', 'MANUEL', 'CARRASCO',DATE '1998-04-12', 456123456, 'MANOLO@GMAIL.COM',TRUE);
